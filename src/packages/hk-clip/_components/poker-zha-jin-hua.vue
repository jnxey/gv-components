...
import { pokerCheckZhaJin } from '@/tools/poker-zha-jin-hua.js';
import { POINTS_GENERAL, POINTS_GENERAL_LIST } from '@/values/index.js';
import { CARD_WIN_DICT, ZHA_JIN_HUA_CARD_TYPE_DICT } from '@/values/card.js';

const emits = defineEmits(['setTypeCompleteInfo']);

const pokerSanGongDictMap = mappingArrayToObject(ZHA_JIN_HUA_CARD_TYPE_DICT, 'value');
...
watch(
  ...
  () => {
    const listMap = props.analysisInfo ?? {};
    const pokerCheck = pokerCheckZhaJin(listMap);
...
.poker-zha-jin-hua .area-box:first-child {
  margin-right: 42px;
}

.poker-zha-jin-hua .area-box .title {
  font-size: 20px;
  color: #ffffff;
  padding: 6px;
}

.poker-zha-jin-hua .poker-box {
  position: relative;
  width: 40px;
  height: 60px;
  cursor: pointer;
...
.poker-zha-jin-hua .poker-box.matched:after {
  content: '';
  box-sizing: border-box;
  position: absolute;
  top: 0;
...
.poker-zha-jin-hua .poker-box img {
  width: 100%;
  height: 100%;
}

.poker-zha-jin-hua .sign-0 {
  position: absolute;
  top: 100px;
  left: 5%;
}

.poker-zha-jin-hua .sign-1 {
  position: absolute;
  top: 100px;
  left: 50%;
...
.poker-zha-jin-hua .sign-2 {
  position: absolute;
  top: 100px;
  right: 5%;
}

.poker-zha-jin-hua .add-btn {
  position: absolute;
  bottom: 8px;
  right: 8px;
  width: 36px;
...
.poker-zha-jin-hua .check-kind .kind-item {
  padding: 5px 8px;
  margin: 0 5px;
  font-size: 12px;
  color: #4caf50;
