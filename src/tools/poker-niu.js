...
// 检查牌点数是否符合规则
export const checkNiuPokerRule = (analysisInfo) => {
  if (analysisInfo.b.length !== 5) return { b: '未识别到牌型' }; // 未识别牌型
  if (analysisInfo.p.length !== 5) return { p: '未识别到牌型' }; // 未识别牌型
  return null;
};
...
export const pokerCheckNiu = (analysisInfo) => {
  try {
    ...
    if (bList.length !== 5 || pList.length !== 5) return { check: false, msg: '请检查扑克牌识别的张数，请检查' };
    // 获取庄闲点数、花色
