...
async function getDevicePorts({ ip, admin, password }) {
  const client = new DigestClient(admin, password);
  const res = await client.fetch(`http://${ip}/ISAPI/System/Network/interfaces`);
 ...
 */
async function captureSnapshot(channelId = 1, subStream = false, { ip, admin, password }) {
  const stream = subStream ? '02' : '01';
  const path = `/ISAPI/Streaming/channels/${channelId}${stream}/picture`;

  const res = await this.client.fetch(this.getUrl(path));
  if (!res.ok) {
